...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, layers = None) -> None:
    model.train()
    criterion.train()
    ...
    for idx, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if layers is not None:
            ...
            if utils.is_dist_avail_and_initialized():
                ...
                active_layers = active_layers_tensor.item()
            outputs = model(samples, active_layers=active_layers)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, layers: int = None):
    model.eval()
    criterion.eval()
    ...
    for active_layers in layers :
        label = f"layers_{active_layers}" if active_layers is not None else "full"
        print(f"\n--- Evaluating [{label}] ---")
