...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, heads = None, effective_heads = None, method="multiforward") -> None:
    model.train()
    criterion.train()
    ...
    for idx, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if heads is not None and method == "multiforward":
            # Single forward pass, compute losses for ALL head configs
            _model = model.module if hasattr(model, 'module') else model
            all_outputs = _model.forward_multi_head(samples, heads)
            ...
            if heads is not None and method == "stochastic_dropout":
            ...
            if idx % print_freq == 0 or idx == len(data_loader) - 1:
                print("Active heads:", effective_heads)
