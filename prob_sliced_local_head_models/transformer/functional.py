...
def _select_heads_projection(
    ...
    # Slice inputs to active heads only
    q = q[:, :, :actual_embed_dim]
    k = k[:, :, :actual_embed_dim]
    v = v[:, :, :actual_embed_dim]

    # Block-diagonal in-projection: head i reads only from its own input block
    # [i*head_dim:(i+1)*head_dim], not from other heads' channels.
    # This matches what a k-head inference run does and avoids cross-head information.
    w_q_view = w_q.view(num_heads, head_dim, embed_dim)
    w_k_view = w_k.view(num_heads, head_dim, embed_dim)
    w_v_view = w_v.view(num_heads, head_dim, embed_dim)
    w_q = torch.block_diag(*[w_q_view[i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
    w_k = torch.block_diag(*[w_k_view[i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
    w_v = torch.block_diag(*[w_v_view[i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
...
def _select_heads_projection_packed(
    ...
    # Block-diagonal in-projection: head i reads only from its own input block
    # [i*head_dim:(i+1)*head_dim], not from other heads' channels.
    w_view = w.view(3, num_heads, head_dim, embed_dim)
    w_bd_q = torch.block_diag(*[w_view[0, i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
    w_bd_k = torch.block_diag(*[w_view[1, i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
    w_bd_v = torch.block_diag(*[w_view[2, i, :, i*head_dim:(i+1)*head_dim] for i in range(effective_heads)])
    w = torch.cat([w_bd_q, w_bd_k, w_bd_v], dim=0)  # [3*actual_embed_dim, actual_embed_dim]
...
def _head_local_input_projection(
    ...
    # w_q/w_k/w_v: [nhead, out_dim, in_dim] — contract over in_dim (d), batch over head (k)
    q_proj = torch.einsum('...kd,kod->...ko', q_h, w_q[:eh])
    k_proj = torch.einsum('...kd,kod->...ko', k_h, w_k[:eh])
    v_proj = torch.einsum('...kd,kod->...ko', v_h, w_v[:eh])
...
def _head_local_output_projection(attn_output, out_w, out_b, effective_heads, head_dim):
    ...
    # out_w: [nhead, out_dim, in_dim] — contract over in_dim (d), batch over head (k)
    out = torch.einsum('nkd,kod->nko', x, out_w[:k])
