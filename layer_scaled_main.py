...
from layer_scaling.training.engine import train_one_epoch, evaluate
...
#python layer_scaled_main.py --temp --hidden_dim 128 --dim_feedforward 1024 --nheads 4 --batch_size 2 --epochs 1 --lr_drop 1 --output_dir ../temp/temp_output --coco_path C:/workspace/ml/data/coco --eval
...
def main(args):
    ...
    if args.enc_layers != args.dec_layers:
        raise Exception("Number of layers need to be the same.")
    

    layers = [i for i in range(1, args.enc_layers + 1, 1)]

    print("Effective layers:", layers)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_max_norm, layers=layers)
        lr_scheduler.step()
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint.pth']
        ...
        test_stats, coco_evaluator = evaluate(
            model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, layers=layers
        )
